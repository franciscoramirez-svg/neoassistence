      ...
      <div className="glass" style={{padding:20,marginBottom:20}}>
        <div style={{display:"flex",gap:12,flexWrap:"wrap",alignItems:"end"}}>
          <div style={{flex:1,minWidth:150}}>
            ...
            <input value={filterEmployee} onChange={e => setFilterEmployee(e.target.value)} placeholder="Filtrar por nombre..." style={{width:"100%",padding:8,borderRadius:8,border:"1px solid rgba(94,242,255,0.2)",background:"rgba(10,21,38,0.8)",color:"white",fontSize:13}} />
          </div>
          <div style={{flex:1,minWidth:150}}>
            <label style={{display:"block",marginBottom:4,color:"#9bb4ca",fontSize:12}}>Fecha</label>
            <input type="date" value={filterDate} onChange={e => setFilterDate(e.target.value)} style={{width:"100%",padding:8,borderRadius:8,border:"1px solid rgba(94,242,255,0.2)",background:"rgba(10,21,38,0.8)",color:"white",fontSize:13}} />
          </div>
          <button onClick={() => { setFilterEmployee(""); setFilterDate(""); }} style={{padding:"8px 14px",borderRadius:8,border:"1px solid rgba(94,242,255,0.2)",background:"transparent",color:"#9bb4ca",cursor:"pointer"}}>Limpiar</button>
        </div>
      </div>
                ...
                {records.map(r => (
                  <tr key={r.id} style={{borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
                    ...
                    <td style={{padding:"8px"}}>
                      {editId === r.id ? (
                        <select value={editData.estatus} onChange={e => setEditData({...editData, estatus: e.target.value})} style={{padding:"4px 8px",borderRadius:6,border:"1px solid rgba(94,242,255,0.2)",background:"rgba(10,21,38,0.8)",color:"white",fontSize:12}}>
                    ...
                    <td style={{padding:"8px"}}>
                      {editId === r.id ? (
                        <textarea value={editData.justificacion} onChange={e => setEditData({...editData, justificacion: e.target.value})} style={{width:"100%",padding:"4px 8px",borderRadius:6,border:"1px solid rgba(94,242,255,0.2)",background:"rgba(10,21,38,0.8)",color:"white",fontSize:12,minHeight:40}} />
                      ) : (
                    ...
                    <td style={{padding:"8px",textAlign:"center"}}>
                      {editId === r.id ? (
                        <div style={{display:"flex",gap:4,justifyContent:"center"}}>
                          <button onClick={saveEdit} disabled={saving} style={{padding:"4px 10px",borderRadius:6,border:"1px solid rgba(156,255,181,0.3)",background:"rgba(156,255,181,0.1)",color:"#9cffb5",fontSize:11,cursor:"pointer"}}>✓</button>
                        ...
                        <button onClick={() => startEdit(r)} style={{padding:"4px 10px",borderRadius:6,border:"1px solid rgba(94,242,255,0.2)",background:"transparent",color:"#5ef2ff",fontSize:11,cursor:"pointer"}}>Editar</button>
                      )}
                    </td>
                  </tr>
                ))}
