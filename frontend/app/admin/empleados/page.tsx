      ...
      <Link href="/dashboard" style={{color:"#00f2fe",textDecoration:"none",display:"block",marginBottom:16}}>← Volver al dashboard</Link>
        ...
        <div className="glass" style={{padding:24,marginBottom:24,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
          ...
          <div style={{display:"flex",gap:8}}>
            ...
            <button onClick={newEmployee} style={{padding:"10px 18px",borderRadius:12,border:"1px solid rgba(0,242,254,0.3)",background:"rgba(0,242,254,0.2)",color:"#00f2fe",fontSize:14,cursor:"pointer"}}>+ Nuevo</button>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className="glass" style={{padding:24}}>
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            <thead>
              <tr style={{borderBottom:"1px solid rgba(0,242,254,0.2)"}}>
            ...
            <tbody>
              {employees.map(emp => (
                <tr key={emp.id} style={{borderBottom:"1px solid rgba(0,242,254,0.1)"}}>
                  ...
                  <td style={{padding:"12px 8px",textAlign:"center",color:"#00f2fe",fontSize:12}}>{(emp.hora_entrada||"??").slice(0,5)}-{(emp.hora_salida||"??").slice(0,5)}</td>
                  <td style={{padding:"12px 8px",textAlign:"center",color:"#9bb4ca",fontSize:12}}>{emp.tolerancia_minutos ?? 15}min</td>
                  <td style={{padding:"12px 8px",textAlign:"center"}}>
                    <span style={{color: emp.activo ? "#b388ff" : "#ff8c9e"}}>{emp.activo ? "Activo" : "Inactivo"}</span>
                  </td>
                  <td style={{padding:"12px 8px",textAlign:"center",display:"flex",gap:8,justifyContent:"center"}}>
                    <button onClick={() => setShowQR(emp.id + "|" + emp.nombre)} style={{padding:"6px 12px",borderRadius:6,border:"1px solid rgba(0,242,254,0.2)",background:"transparent",color:"#00f2fe",fontSize:12}}>QR</button>
                    <button onClick={() => { setFaceEmp(emp.id); setFaceEmpName(emp.nombre); setFaceRegistered(false); setFaceDetected(false); }} style={{padding:"6px 12px",borderRadius:6,border:"1px solid " + (emp.face_descriptor ? "rgba(179,136,255,0.4)" : "rgba(255,140,158,0.3)"),background: emp.face_descriptor ? "rgba(179,136,255,0.1)" : "transparent",color: emp.face_descriptor ? "#b388ff" : "#ff8c9e",fontSize:12}}>{emp.face_descriptor ? "✓ Rostro" : "✗ Rostro"}</button>
                    <button onClick={() => editEmployee(emp)} style={{padding:"6px 12px",borderRadius:6,border:"1px solid rgba(0,242,254,0.2)",background:"transparent",color:"#00f2fe",fontSize:12}}>Editar</button>
                    <button onClick={() => toggleActivo(emp)} style={{padding:"6px 12px",borderRadius:6,border:"1px solid " + (emp.activo ? "rgba(255,140,158,0.3)" : "rgba(179,136,255,0.3)"),background:"transparent",color: emp.activo ? "#ff8c9e" : "#b388ff",fontSize:12}}>
...
{editMode && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.9)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100,overflow:"auto"}}>
          <div className="glass" style={{maxWidth:340,width:"85%",padding:16,borderRadius:12,margin:20}}>
            <h2 style={{color:"#00f2fe",marginTop:0,fontSize:16}}>{selectedEmp ? "Editar" : "Nuevo"} Empleado</h2>
            
            <div style={{marginBottom:10}}>
              <label style={{display:"block",marginBottom:4,color:"#9bb4ca",fontSize:12}}>Nombre</label>
              <input value={formData.nombre} onChange={e => setFormData({...formData, nombre: e.target.value})} style={{width:"100%",padding:10,borderRadius:8,border:"1px solid rgba(0,242,254,0.2)",background:"rgba(10,21,38,0.8)",color:"white",fontSize:14}} />
            </div>

            <div style={{marginBottom:10}}>
              <label style={{display:"block",marginBottom:4,color:"#9bb4ca",fontSize:12}}>No. Empleado</label>
              <input value={formData.numero_empleado} onChange={e => setFormData({...formData, numero_empleado: e.target.value})} style={{width:"100%",padding:10,borderRadius:8,border:"1px solid rgba(0,242,254,0.2)",background:"rgba(10,21,38,0.8)",color:"white",fontSize:14}} />
            </div>

            <div style={{marginBottom:10}}>
              <label style={{display:"block",marginBottom:4,color:"#9bb4ca",fontSize:12}}>PIN</label>
              <div style={{display:"flex",gap:8}}>
                <input value={formData.pin} onChange={e => setFormData({...formData, pin: e.target.value})} style={{flex:1,padding:10,borderRadius:8,border:"1px solid rgba(0,242,254,0.2)",background:"rgba(10,21,38,0.8)",color:"white",fontSize:14,fontFamily:"monospace"}} />
            ...
            <div style={{marginBottom:10}}>
              ...
              <input value={formData.rol} onChange={e => setFormData({...formData, rol: e.target.value})} placeholder="Ej: Tecnico Electronico, Andamiero, Supervisor..." style={{width:"100%",padding:10,borderRadius:8,border:"1px solid rgba(0,242,254,0.2)",background:"rgba(10,21,38,0.8)",color:"white",fontSize:14}} />
            </div>

            <div style={{marginBottom:10}}>
              <label style={{display:"block",marginBottom:4,color:"#9bb4ca",fontSize:12}}>Sucursal</label>
              <select value={formData.sucursal_id} onChange={e => setFormData({...formData, sucursal_id: e.target.value})} style={{width:"100%",padding:10,borderRadius:8,border:"1px solid rgba(0,242,254,0.2)",background:"rgba(10,21,38,0.8)",color:"white",fontSize:14}}>
            ...
            {selectedEmp && (
              <div style={{marginBottom:10}}>
                ...
                <button onClick={uploadPhoto} disabled={uploadingPhoto} style={{padding:"6px 12px",borderRadius:6,border:"1px solid rgba(0,242,254,0.2)",background:"transparent",color:"#00f2fe",fontSize:12}}>
                  {uploadingPhoto ? "Subiendo..." : "Subir Foto"}
                </button>
                {selectedEmp.foto_url && <span style={{color:"#b388ff",fontSize:12,marginLeft:8}}>✓ tiene foto</span>}
              </div>
            )}
            ...
            <div style={{display:"flex",gap:8}}>
                <div style={{flex:1}}>
                  ...
                  <input type="time" value={formData.hora_entrada} onChange={e => setFormData({...formData, hora_entrada: e.target.value})} style={{width:"100%",padding:8,borderRadius:8,border:"1px solid rgba(0,242,254,0.2)",background:"rgba(10,21,38,0.8)",color:"white",fontSize:13}} />
                </div>
                <div style={{flex:1}}>
                  <span style={{color:"#9bb4ca",fontSize:11}}>Salida</span>
                  <input type="time" value={formData.hora_salida} onChange={e => setFormData({...formData, hora_salida: e.target.value})} style={{width:"100%",padding:8,borderRadius:8,border:"1px solid rgba(0,242,254,0.2)",background:"rgba(10,21,38,0.8)",color:"white",fontSize:13}} />
                </div>
                <div style={{flex:1}}>
                  <span style={{color:"#9bb4ca",fontSize:11}}>Tol. (min)</span>
                  <input type="number" value={formData.tolerancia_minutos} onChange={e => setFormData({...formData, tolerancia_minutos: e.target.value})} style={{width:"100%",padding:8,borderRadius:8,border:"1px solid rgba(0,242,254,0.2)",background:"rgba(10,21,38,0.8)",color:"white",fontSize:13}} min="0" max="120" />
                </div>
                <div style={{flex:1}}>
                  <span style={{color:"#9bb4ca",fontSize:11}}>$ Diario</span>
                  <input type="number" value={formData.sueldo_diario} onChange={e => setFormData({...formData, sueldo_diario: e.target.value})} style={{width:"100%",padding:8,borderRadius:8,border:"1px solid rgba(0,242,254,0.2)",background:"rgba(10,21,38,0.8)",color:"white",fontSize:13}} min="0" step="10" />
                </div>
              </div>
            </div>
            ...
            <div style={{display:"flex",gap:8}}>
              <button onClick={saveEmployee} style={{flex:1,padding:10,borderRadius:8,border:"1px solid rgba(0,242,254,0.3)",background:"rgba(0,242,254,0.2)",color:"white",fontWeight:"bold",fontSize:13}}>Guardar</button>
      ...
      {showQR && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.9)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100}}>
          <div className="glass" style={{padding:20,borderRadius:12,textAlign:"center"}}>
            <h2 style={{color:"#00f2fe",marginTop:0,fontSize:16}}>Código QR</h2>
            ...
            <button onClick={() => setShowQR(null)} style={{padding:"8px 16px",borderRadius:8,border:"1px solid rgba(0,242,254,0.3)",background:"rgba(0,242,254,0.2)",color:"white",fontSize:13}}>Cerrar</button>
          </div>
        </div>
      )}
      ...
      {faceEmp && !faceRegistered && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.9)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100}}>
          <div className="glass" style={{maxWidth:320,width:"85%",padding:16,borderRadius:12}}>
            <h2 style={{color:"#00f2fe",marginTop:0,fontSize:16}}>Registrar Rostro</h2>
            <p style={{color:"#b388ff",fontSize:14,marginBottom:10}}>{faceEmpName}</p>
            <div style={{borderRadius:8,overflow:"hidden",background:"#000",marginBottom:10,height:240,display:"flex",alignItems:"center",justifyContent:"center",border: faceDetected ? "2px solid rgba(179,136,255,0.6)" : "2px solid transparent",boxShadow: faceDetected ? "0 0 12px rgba(179,136,255,0.3)" : "none",transition:"all 0.3s"}}>
            ...
            <p style={{color:faceDetected?"#b388ff":"#9bb4ca",fontSize:13,textAlign:"center",marginBottom:12}}>
              {faceDetected ? "✓ Rostro detectado" : "Coloca tu rostro frente a la cámara"}
            </p>
            <div style={{display:"flex",gap:8}}>
              <button onClick={registerFace} disabled={!faceDetected || savingFace} style={{flex:1,padding:10,borderRadius:8,border:"1px solid rgba(179,136,255,0.3)",background:faceDetected?"rgba(179,136,255,0.2)":"#1a2a3a",color:"white",fontWeight:"bold",fontSize:13,opacity:faceDetected?1:0.5}}>
      ...
      {faceEmp && faceRegistered && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.9)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100}}>
          <div className="glass" style={{padding:24,borderRadius:12,textAlign:"center"}}>
            ...
            <h2 style={{color:"#b388ff",margin:0,fontSize:16}}>Rostro registrado</h2>
            <p style={{color:"#9bb4ca",marginTop:6,fontSize:13}}>El rostro se ha guardado correctamente</p>
            <button onClick={() => { closeFaceCamera(); setFaceEmp(null); setFaceRegistered(false); }} style={{marginTop:12,padding:"8px 16px",borderRadius:8,border:"1px solid rgba(0,242,254,0.3)",background:"rgba(0,242,254,0.2)",color:"white",fontSize:13}}>Cerrar</button>
          </div>
        </div>
      )}
    </main>
