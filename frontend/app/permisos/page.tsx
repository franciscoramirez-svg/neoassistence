    ...
    <main className="page-shell">
      <nav style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 0",marginBottom:8}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          <Link href={user?.role === "admin" ? "/dashboard" : user?.role?.includes("supervisor") ? "/supervisor" : "/empleado"} style={{color:"#5ef2ff",fontSize:13,textDecoration:"none"}}>← Volver</Link>
      ...
      <section className="glass" style={{padding:24,marginBottom:20}}>
        <p style={{color:"#5ef2ff",textTransform:"uppercase",letterSpacing:"0.18em"}}>Permisos y Vacaciones</p>
      ...
      {showForm && (
        <div className="glass" style={{padding:20,marginBottom:20,borderRadius:16}}>
          <h3 style={{margin:"0 0 12px",color:"#5ef2ff",fontSize:14}}>Nueva solicitud</h3>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(140px, 1fr))",gap:10,marginBottom:12}}>
            ...
            <div>
              ...
              <input type="date" value={formData.fecha_inicio} onChange={e => setFormData({...formData, fecha_inicio: e.target.value})} style={{width:"100%",padding:8,borderRadius:8,border:"1px solid rgba(94,242,255,0.2)",background:"rgba(10,21,38,0.8)",color:"white",fontSize:13}} />
            </div>
            <div>
              <label style={{display:"block",marginBottom:4,color:"#9bb4ca",fontSize:12}}>Fin</label>
              <input type="date" value={formData.fecha_fin} onChange={e => setFormData({...formData, fecha_fin: e.target.value})} style={{width:"100%",padding:8,borderRadius:8,border:"1px solid rgba(94,242,255,0.2)",background:"rgba(10,21,38,0.8)",color:"white",fontSize:13}} />
            </div>
          </div>
                ...
                {permisos.map(p => (
                  <tr key={p.id} style={{borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
                    ...
                    <td style={{padding:"8px",textAlign:"center"}}>
                      <span style={{color: p.estatus === "aprobado" ? "#9cffb5" : p.estatus === "rechazado" ? "#ff8c9e" : "#ffcc5e"}}>
                    ...
                    {user?.role === "admin" && p.estatus === "pendiente" && (
                      <td style={{padding:"8px",textAlign:"center"}}>
                        <div style={{display:"flex",gap:6,justifyContent:"center"}}>
                          <button onClick={() => handleResolve(p.id, "aprobado")} style={{padding:"4px 10px",borderRadius:6,border:"1px solid rgba(156,255,181,0.3)",background:"rgba(156,255,181,0.1)",color:"#9cffb5",fontSize:11,cursor:"pointer"}}>✓</button>
